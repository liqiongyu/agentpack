## 1. Spec (OpenSpec)
- [x] Define `agentpack overlay edit --kind patch` behavior in the delta spec under `openspec/changes/archive/2026-01-14-add-patch-overlay-edit/specs/agentpack-cli/spec.md`.
...
- [x] Archive the change via `openspec archive add-patch-overlay-edit --yes` in a separate PR after the implementation is merged.
